...
const MEAL_PROMPT =
  'You are a nutrition expert analyzing a meal. First, identify ALL ingredients you can see (be specific: type of protein, vegetables, grains, sauces). Then estimate protein and macros based on typical home portion sizes. Do not overestimate. When uncertain, use the lower value. Return JSON only: {"meal_name": "detailed description of ingredients, e.g. black rice with shrimp, spinach and asparagus", "calories": number, "protein_grams": number, "fat_grams": number, "carb_grams": number, "fiber_grams": number, "confidence": "low/medium/high"}. The meal_name must describe actual ingredients specifically, never use generic names like mixed plate, mixed dish, or meal. If input text is provided, keep the same dish name as input.';
...
export async function analyzeMealWithGemini({ type, content, caption = "" }) {
  const apiKey = envOrThrow("OPENROUTER_API_KEY");
  const note = String(caption || "").trim();
  const imagePrompt = note ? `${MEAL_PROMPT}\n\nUser note: ${note}` : MEAL_PROMPT;
  ...
  for (const model of OPENROUTER_MODELS) {
    try {
      ...
      if (!res.ok) {
        ...
        const err = new Error(`OpenRouter error (${model}): ${res.status} ${errorText}`);
        lastError = err;
      ...
      lastError = e;
      errors.push(`Model ${model} failed: ${e.message || e}`);
  ...
  if (errors.length > 0) {
    throw new Error(`Все модели недоступны. Последние ошибки: ${errors.slice(-3).join(" | ")}`);
  }
